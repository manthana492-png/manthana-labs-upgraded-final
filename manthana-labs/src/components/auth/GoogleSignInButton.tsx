...
interface Props {
  /** Where to send the user after successful Google sign-in. Defaults to current origin. */
  redirectTo?: string;
  label?: string;
  className?: string;
}

/** "Continue with Google" button — managed Lovable Cloud OAuth. */
export const GoogleSignInButton = ({
  ...
  return (
    <Button
      ...
      onClick={onClick}
      disabled={busy}
      className={`w-full h-11 bg-background hover:bg-surface-raised gap-2 ${className ?? ""}`}
    >
      {busy ? (
